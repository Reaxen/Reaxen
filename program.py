...
def YoklamaAlma():
    ...
    dort_col = ws.col(2)
    dort_col.width = 256 * 20

    bes_col = ws.col(2)
    bes_col.width = 256 * 20

    alti_col = ws.col(2)
    alti_col.width = 256 * 20

    df = pd.read_excel('ogrenciler.xlsx')
    ad = df['Ad']
    soyad = df['Soyad']
    no = df['Numara']
    ...
    ws.write(0, 3, datetime.datetime.now(), style1)
    ws.write(0, 0, 'Numara', style=style)
    ws.write(0, 1, 'Ad', style=style)
    ws.write(0, 2, 'Soyad', style=style)
    ws.write(0, 4, 'Giris', style=style)
    ws.write(0, 5, 'Cıkıs', style=style)
    ws.write(0, 6, 'Toplam süre', style=style)
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            for i in range(0, uzunluk):
                ...
                ay = time.strftime("%m", named_tuple)
                gün = time.strftime("%d", named_tuple)
                yıl = time.strftime("%Y", named_tuple)
                saat = time.strftime("%H", named_tuple)
                dakika = time.strftime("%M", named_tuple)
                saniye = time.strftime("%S", named_tuple)
                print(time_string)
                print(ay)
                print(gün)
                print(yıl)
                f_date = datetime.datetime(year=int(yıl),month=int(ay),day=int(gün),hour=int(saat),minute=int(dakika),second=int(saniye))
                if Id == no[i]:
                    named_tuple = time.localtime()  # get struct_time
                    time_string = time.strftime("%m/%d/%Y, %H:%M:%S", named_tuple)
                    ay = time.strftime("%m", named_tuple)
                    ...
                    ws.write(i + 1, 4, str(saat+":"+dakika+ ":" +saniye), style=style)

                ws.write(i + 1, 5, str(saat+":"+dakika+ ":" +saniye), style=style)
                delta = str(int(saat)- int(time.strftime("%H", time.localtime()))) +":"+str(int(dakika)- int(time.strftime("%M", time.localtime())))+ ":"+str(int(saniye)- int(time.strftime("%S", time.localtime())))
                ws.write(i + 1, 6, str(delta), style=style)
        ...
        if (cv2.waitKey(1) == ord('q')):
            wb.save('data/' + str(datetime.date.today()) + '.xls')
            break
    cam.release()
...
def YeniOgrenciKaydet():
    ...
    wb = load_workbook('ogrenciler.xlsx')
